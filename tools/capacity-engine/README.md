...
| **Render** | `render.go`, `render_manifests.go` | Writes `capacity-output.yaml` and patches Kubernetes manifests. |
...
#### Kingman's Formula (G/G/1) — *Kingman, 1961*
...
```
*Pool of 1 serializes — floor exists so pools never collapse to 1 (HikariCP/PG wiki).*
...
```
*Consumers = `min_replicas`; growth headroom covers 1–2 yr traffic growth.*
...
```
*Memory constants (`models.go` → `constants.go`): 50 KB per PG conn (pgx), 50 KB per TLS conn (net/http Transport), 64 MiB Go runtime baseline.*
...
3. **Kubernetes manifests** — patches `base/workloads/config/` ConfigMaps, KEDA ScaledObject thresholds, and Kafka topic partition counts, making output immediately deployable via GitOps.
...
| File | Purpose |
|------|---------|
| `models.go` | **All mathematical models** — Kingman, Erlang C, PG connections, Kafka caps, Redis, retry, session, resource sizing |
| `constants.go` | Physical constants (Kafka partition limits, per-connection memory overhead) |
| `supply.go` | Computes infrastructure supply ceilings from physical inputs |
| `demand.go` | Pure orchestration — calls models.go per service |
| `fitcheck.go` | Validates demand ≤ supply, clamps HPA caps, emits failures/warnings |
| `render.go` | Writes `capacity-output.yaml` and terminal report |
| `render_manifests.go` | Patches Kubernetes ConfigMaps, KEDA ScaledObjects, Kafka topic manifests |
| `main.go` | Entry point: supply → demand → fitcheck → render pipeline |
| `types.go` | All input (`SLOInput`) and output (`Derived`, `PGCeiling`, etc.) type definitions |
| `slo-input.yaml` | The authoritative input file (360 lines of annotated infrastructure specs) |
| `capacity-output.yaml` | Generated debug output (do not edit manually) |
